...
const portfolioData = {
  ...
  featured: [
    { name: "Startsmart", role: "Side Project", description: "Know if your business idea is worth starting.", url: "/coming-soon?project=Startsmart", image: "/startsmart-logo.png" },
    { name: "PropertyHub", role: "Side Project", description: "Find and compare properties across Nigeria without switching platforms.", url: "/coming-soon?project=PropertyHub", image: "/propertyhub-logo.png" }
  ],
  projects: [
          ...
          {portfolioData.featured.map((project, index) => (
            ...
            >
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-secondary shrink-0 flex items-center justify-center text-base sm:text-lg font-semibold text-muted-foreground overflow-hidden">
                {project.image ? (
                  <Image src={project.image} alt={project.name} fill className="object-cover" sizes="96px" quality={100} />
                ) : (
                  project.name[0]
                )}
              </div>
              ...
              <ArrowRight 
                size={14} 
                className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 text-muted-foreground" 
              />
            </Link>
          ))}
        </div>
